...
func (c *TelegramCalls) downloadAndPrepareSong(bot *td.Client, song *utils.CachedTrack, reply *td.Message) error {
	...
	dlPath, err := dl.AcquireCachedTrack(song, bot)
...
func (c *TelegramCalls) handleAutoplay(bot *td.Client, chatID int64, lastSong *utils.CachedTrack) error {
	...
	tracks, err := dl.FetchYouTubeMixPlaylist(ctx, playlistID)
...
// processFloodEvent manages flood wait errors by pausing execution for short waits.
// It sleeps only if the wait is <= 5 seconds. Otherwise, it returns false.
func processFloodEvent(err error) bool {
...
func loadVideoDimensions(filePath string) (int, int) {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	cmd := exec.CommandContext(ctx, "ffprobe", "-v", "error", "-select_streams", "v:0", "-show_entries", "stream=width,height", "-of", "csv=s=x:p=0", filePath)
	out, err := cmd.Output()
	if err != nil {
		logger.Warn("[loadVideoDimensions] Failed to get video dimensions (%s): %v", filePath, err)
		return 0, 0
	}
	dimensions := strings.Split(strings.TrimSpace(string(out)), "x")
	if len(dimensions) != 2 {
		logger.Warn("[loadVideoDimensions] Invalid video dimensions(%s): %s", filePath, string(out))
		return 0, 0
	}
