...
import (
	...
	"github.com/BabiesIQ/SPOTIFY_MUSIC/player/sessions"
	"errors"
	"fmt"
	"log/slog"
	"strconv"
...
func (c *TelegramCalls) recoverBannedAssistant(bot *td.Client, chatID int64, call *Assistant, index int, isBanned bool) error {
	ubID := call.App.Me().ID
	botStatus, err := cache.LoadUserAdmin(bot, chatID, bot.Me.Id, false)
	if err != nil {
...
func (c *TelegramCalls) StartClient(apiID int32, apiHash, stringSession string) (*Assistant, error) {
	...
	clientConfig := tg.ClientConfig{
		...
		FloodHandler:  processFloodEvent,
		LogLevel:      tg.InfoLevel,
	}

	switch config.SessionType {
	case "telethon":
		sess, err = sessions.ParseTelethonSession(stringSession)
		if err != nil {
	...
	case "pyrogram":
		sess, err = sessions.ParsePyrogramSession(stringSession)
		if err != nil {
	...
	call, err := createAssistant(mtProto)
	if err != nil {
		_ = mtProto.Stop()
