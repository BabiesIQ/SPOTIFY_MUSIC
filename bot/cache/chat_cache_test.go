...
func buildTrack(id, name string) *utils.CachedTrack {
...
func createCache() *ChatCacher {
	return createChatCacher()
}
...
func TestAddSong_Single(t *testing.T) {
	c := createCache()
	n := c.AddSong(1, buildTrack("t1", "Track 1"))
	if n != 1 {
...
func TestAddSong_Multiple(t *testing.T) {
	c := createCache()
	c.AddSong(1, buildTrack("t1", "Track 1"))
	n := c.AddSong(1, buildTrack("t2", "Track 2"))
	if n != 2 {
...
func TestAddSong_DifferentChats(t *testing.T) {
	c := createCache()
	c.AddSong(1, buildTrack("t1", "Track 1"))
	n := c.AddSong(2, buildTrack("t2", "Track 2"))
	if n != 1 {
...
func TestAddSongs(t *testing.T) {
	...
	tracks := []*utils.CachedTrack{
		buildTrack("t1", "Track 1"),
		buildTrack("t2", "Track 2"),
		buildTrack("t3", "Track 3"),
	}
...
func TestAddSongs_AppendToExisting(t *testing.T) {
	c := createCache()
	c.AddSong(1, buildTrack("t0", "Track 0"))
...
func TestGetPlayingTrack_ReturnsFirst(t *testing.T) {
	c := createCache()
	c.AddSong(1, buildTrack("t1", "Track 1"))
	c.AddSong(1, buildTrack("t2", "Track 2"))
...
func TestRemoveTrack_ValidIndex(t *testing.T) {
	...
	c.AddSong(1, buildTrack("t1", "Track 1"))
	c.AddSong(1, buildTrack("t2", "Track 2"))
	c.AddSong(1, buildTrack("t3", "Track 3"))
...
func TestGetActiveChats_Multiple(t *testing.T) {
	c := createCache()
	c.AddSong(1, buildTrack("t1", "Track 1"))
	c.AddSong(2, buildTrack("t2", "Track 2"))
	c.AddSong(3, buildTrack("t3", "Track 3"))
	c.ClearChat(2)
...
func TestConcurrentAddAndRead(t *testing.T) {
	...
	for i := 0; i < goroutines; i++ {
		...
		go func(i int) {
			defer wg.Done()
			id := int64(i % 5)
			c.AddSong(id, buildTrack("t", "Track"))
...
func TestConcurrentAddAndRemove(t *testing.T) {
	...
	for i := 0; i < 20; i++ {
		...
		go func() {
			defer wg.Done()
			c.AddSong(1, buildTrack("t", "Track"))
		}()
	}
	for i := 0; i < 20; i++ {
		wg.Add(1)
