...
func removeHandler(c *td.Client, m *td.Message) error {
	...
	if len(sortedTracks) == 1 {
		_, err = m.ReplyText(c, fmt.Sprintf("Track #%d has been removed by %s.", sortedTracks[0], userFirstName(c, m)), replyOpts)
	} else {
		_, err = m.ReplyText(c, fmt.Sprintf("%d tracks have been removed by %s.", len(sortedTracks), userFirstName(c, m)), replyOpts)
	}
