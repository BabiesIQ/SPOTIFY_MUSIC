...
func verifyBotAdmin(c *td.Client, chatID int64, replyErr func(msg string)) bool {
	botStatus, err := cache.LoadUserAdmin(c, chatID, c.Me.Id, false)
	if err != nil {
...
func adminModeState(c *td.Client, m *td.Message) bool {
	...
	if !verifyBotAdmin(c, chatID, func(msg string) { _, _ = m.ReplyText(c, msg, nil) }) {
		return false
	}
...
func adminModeCallback(c *td.Client, cb *td.UpdateNewCallbackQuery) bool {
	if cb.IsPrivate() {
		return false
	}
	...
	if !verifyBotAdmin(c, chatID, func(msg string) { _ = cb.Answer(c, 0, true, msg, "") }) {
		return false
	}
