...
// processParticipant processes every chat-member status change for the bot and its assistant.
func processParticipant(client *gotdbot.Client, update *gotdbot.UpdateChatMember) error {
	...
	if !isBotAccount(userID, client.Me.Id, assistantID) {
		return gotdbot.EndGroups
	}

	chat, err := retrieveSupergroup(client, chatID)
	if err != nil {
		return gotdbot.EndGroups
	}

	if chat == nil {
		// retrieveSupergroup already left the chat when appropriate.
		return gotdbot.EndGroups
	}

	if sanitizedUsername(chat) != "" {
		vc.Calls.UpdateInviteLink(chatID, "https://t.me/"+sanitizedUsername(chat))
	}
	...
	if hasAdminRights(oldStatus) || hasAdminRights(newStatus) {
	...
	return emitStatusChange(client, chatID, userID, assistantID, oldStatus, newStatus, chat)
}

// emitStatusChange routes a member-status transition to the right handler.
...
func onRestriction(
	...
	case wasRestricted && !nowRestricted:
		...
		refreshMembershipCache(chatID, userID, newStatus)
...
func onJoin(
	...
	refreshMembershipCache(chatID, userID, &gotdbot.ChatMemberStatusMember{})
	return nil
}
...
func onLeave(client *gotdbot.Client, chatID, userID, assistantID int64) error {
	...
	refreshMembershipCache(chatID, userID, &gotdbot.ChatMemberStatusLeft{})
	if userID == client.Me.Id {
...
func onBan(client *gotdbot.Client, chatID, userID, assistantID int64) error {
	...
	refreshMembershipCache(chatID, userID, &gotdbot.ChatMemberStatusBanned{})
...
func onPromotionOrDemotion(
	...
	case !wasAdmin && nowAdmin:
		...
		refreshMembershipCacheByID(chatID, userID, &gotdbot.ChatMemberStatusAdministrator{})

	case wasAdmin && !nowAdmin:
		client.Logger.Info("User demoted in chat", "user_id", userID, "chat_id", chatID)
		refreshMembershipCacheByID(chatID, userID, &gotdbot.ChatMemberStatusMember{})
...
// retrieveSupergroup fetches the Supergroup for a chat ID.
// Returns nil (and leaves the chat) when the chat should be abandoned.
func retrieveSupergroup(client *gotdbot.Client, chatID int64) (*gotdbot.Supergroup, error) {
...
func sanitizedUsername(chat *gotdbot.Supergroup) string {
...
// isBotAccount reports whether the given user is the bot or its assistant.
func isBotAccount(userID, botID, assistantID int64) bool {
	return userID == botID || userID == assistantID
}

// hasAdminRights reports whether a ChatMemberStatus is admin-level.
func hasAdminRights(status gotdbot.ChatMemberStatus) bool {
	switch status.(type) {
...
// refreshMembershipCache updates the VC membership for the assistant only.
func refreshMembershipCache(chatID, userID int64, status gotdbot.ChatMemberStatus) {
	vc.Calls.UpdateMembership(chatID, userID, status)
}

// refreshMembershipCacheByID is like refreshMembershipCache but resolves the
func refreshMembershipCacheByID(chatID, userID int64, status gotdbot.ChatMemberStatus) {
	vc.Calls.UpdateMembership(chatID, userID, status)
}
