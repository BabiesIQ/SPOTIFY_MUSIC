...
func playHandler(c *td.Client, m *td.Message) error {
	...
	return processPlay(c, m, false, false)
}

// playVideoHandler handles the /vplay command.
func playVideoHandler(c *td.Client, m *td.Message) error {
	if !playMode(c, m) {
		return td.EndGroups
	}
	...
	return processPlay(c, m, true, false)
}

// playFileHandler handles the /fplay command.
func playFileHandler(c *td.Client, m *td.Message) error {
	if !adminModeState(c, m) {
		return td.EndGroups
	}

	return processPlay(c, m, false, true)
}

// playVideoFileHandler handles the /fvplay command.
func playVideoFileHandler(c *td.Client, m *td.Message) error {
	if !adminModeState(c, m) {
		return td.EndGroups
	}
	...
	return processPlay(c, m, true, true)
}

func processPlay(c *td.Client, m *td.Message, isVideo bool, force bool) error {
	chatID := m.ChatId
	...
	if strings.HasPrefix(input, "tgpl_") {
		...
		tracks := db.TransformSongsToTracks(playlist.Songs)
		if len(tracks) == 0 {
		...
		return processMultipleTracks(c, m, updater, tracks, chatID, isVideo, force)
	}

	if match := utils.TelegramMessageRegex.FindStringSubmatch(input); match != nil {
		rMsg, err = utils.LoadMessage(c, input)
		if err != nil {
	...
	if isValid := isMediaValid(rMsg); isValid {
		isReply = true
	}

	if url == "" && args == "" && (!isReply || !isMediaValid(rMsg)) {
	...
	if isReply && isMediaValid(rMsg) {
		return processMedia(c, m, updater, rMsg, chatID, isVideo, force)
	}

	wrapper := dl.CreateDownloaderWrapper(input)
	if url != "" {
		if !wrapper.IsValid() {
		...
		return processUrl(c, m, updater, trackInfo, chatID, isVideo, force)
	}

	return processTextSearch(c, m, updater, wrapper, chatID, isVideo, force)
}

// processMedia handles playing media from a message.
func processMedia(c *td.Client, m *td.Message, updater *td.Message, dlMsg *td.Message, chatId int64, isVideo bool, force bool) error {
	file, fileName := loadFile(dlMsg)
	if file == nil {
	...
	dur := utils.LoadFileDuration(dlMsg)
	...
	saveCache := utils.CachedTrack{
		URL: link.Link, Name: fileName, User: userFirstName(c, m), TrackID: fileId,
	...
	if dur == 0 {
		dur = utils.LoadMediaDuration(filePath)
	...
	_, err = updater.EditText(c, nowPlaying, &td.EditTextMessageOpts{
		ParseMode:             "HTML",
		ReplyMarkup:           core.PlayerControlButtons("play"),
...
func processTextSearch(c *td.Client, m *td.Message, updater *td.Message, wrapper *dl.DownloaderWrapper, chatId int64, isVideo bool, force bool) error {
	...
	return processSingleTrack(c, m, updater, song, "", chatId, isVideo, force)
}

// processUrl handles a URL search for a song.
func processUrl(c *td.Client, m *td.Message, updater *td.Message, trackInfo utils.PlatformTracks, chatId int64, isVideo bool, force bool) error {
	if len(trackInfo.Results) == 1 {
		...
		return processSingleTrack(c, m, updater, track, "", chatId, isVideo, force)
	}

	return processMultipleTracks(c, m, updater, trackInfo.Results, chatId, isVideo, force)
}

// processSingleTrack handles a single track.
func processSingleTrack(c *td.Client, m *td.Message, updater *td.Message, song utils.MusicTrack, filePath string, chatId int64, isVideo bool, force bool) error {
	...
	saveCache := utils.CachedTrack{
		URL: song.Url, Name: song.Title, User: userFirstName(c, m), FilePath: filePath,
	...
	if saveCache.FilePath == "" {
		dlResult, err := dl.AcquireCachedTrack(&saveCache, c)
		if err != nil {
			cache.ChatCache.RemoveCurrentSong(chatId)
			_, err = updater.EditText(c, fmt.Sprintf("Download failed: %s", err.Error()), nil)
			return err
...
// processMultipleTracks handles multiple tracks.
func processMultipleTracks(c *td.Client, m *td.Message, updater *td.Message, tracks []utils.MusicTrack, chatId int64, isVideo bool, force bool) error {
	if len(tracks) == 0 {
	...
	for _, track := range tracks {
		...
		saveCache := &utils.CachedTrack{
			...
			Thumbnail: track.Thumbnail, User: userFirstName(c, m), Platform: track.Platform,
	...
	escRequester := html.EscapeString(userFirstName(c, m))
