...
// vcActiveHandler handles the /activevc command.
// It takes a telegram.NewMessage object as input.
// It returns an error if any.
func vcActiveHandler(c *td.Client, m *td.Message) error {
	if !isDeveloper(c, m) {
		return td.EndGroups
	}
...
func assistantsClearHandler(c *td.Client, m *td.Message) error {
	if !isDeveloper(c, m) {
		return td.EndGroups
	}
...
func allLeaveHandler(c *td.Client, m *td.Message) error {
	if !isDeveloper(c, m) {
		return td.EndGroups
	}
...
func logController(c *td.Client, m *td.Message) error {
	if !isDeveloper(c, m) {
		return td.EndGroups
	}
