...
func seekHandler(c *td.Client, m *td.Message) error {
	...
	_, _ = m.ReplyText(c, fmt.Sprintf("<b>Stream skipped %s and started from %s seconds by</b> %s", utils.SecToMin(seekTime), utils.SecToMin(toSeek), userFirstName(c, m)), replyOpts)
	return nil
}
