...
func userFirstName(c *gotdbot.Client, m *gotdbot.Message) string {
	if m.SenderId == nil {
		return "Unknown"
	}
...
// isDeveloper checks if the user is a developer.
// It returns true if the user is a developer, otherwise false.
func isDeveloper(c *gotdbot.Client, m *gotdbot.Message) bool {
...
// resolveTargetUserID resolves a target user ID from a reply or command arguments.
// Resolution order: replied message → numeric ID → @username lookup.
func resolveTargetUserID(c *gotdbot.Client, m *gotdbot.Message) (int64, error) {
...
// retrieveFormattedDuration returns a human-readable string for the given duration.
func retrieveFormattedDuration(diff time.Duration) string {
