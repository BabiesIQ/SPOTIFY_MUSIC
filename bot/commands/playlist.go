...
func playlistCreateHandler(c *td.Client, m *td.Message) error {
...
func playlistDeleteHandler(c *td.Client, m *td.Message) error {
...
func playlistAddHandler(c *td.Client, m *td.Message) error {
	...
	wrapper := dl.CreateDownloaderWrapper(songURL)
	if !wrapper.IsValid() {
		_, err := m.ReplyText(
			c,
...
func userPlaylistsHandler(c *td.Client, m *td.Message) error {
