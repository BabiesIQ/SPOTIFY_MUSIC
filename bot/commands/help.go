...
func loadHelpCategories() map[string]struct {
	Title   string
	Content string
...
<details>
  ...
  <table bordered striped>
    ...
    <tr><td><code>/queue</code></td><td>Display the current playback queue.</td></tr>
			...
			Markup: core.HelpMenuBackKeyboard(),
		},
		"help_admin": {
...
func callbackHelpHandler(c *td.Client, cb *td.UpdateNewCallbackQuery) error {
	data := cb.DataString()
	...
	helpCategories := loadHelpCategories()
	...
	if strings.Contains(data, "help_all") {
		...
		_, _ = c.EditMessageText(cb.ChatId, &td.InputMessageRichMessage{Message: richMessage}, cb.MessageId, &td.EditMessageTextOpts{ReplyMarkup: core.BuildHelpMenuKeyboard()})
		return nil
	}
	...
	if strings.Contains(data, "help_back") {
		...
		_, _ = c.EditMessageText(cb.ChatId, &td.InputMessageRichMessage{Message: richMessage}, cb.MessageId, &td.EditMessageTextOpts{ReplyMarkup: core.JoinButtonMarkup(c.Me.Usernames.EditableUsername)})
		return nil
	}
