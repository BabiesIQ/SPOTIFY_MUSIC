...
// LoadFileDuration extracts the duration of a media file from a Telegram message.
func LoadFileDuration(m *td.Message) int {
	if m.Content == nil {
		return 0
	}
...
// LoadMediaDuration returns duration in seconds (int).
func LoadMediaDuration(input string) int {
