...
// LoadMessage retrieves a Telegram message by its URL.
func LoadMessage(client *td.Client, url string) (*td.Message, error) {
	url = strings.TrimSpace(url)
	if url == "" {
		return nil, errors.New("url must not be empty")
	}

	link, err := constructMessageLink(url)
	if err != nil {
		return nil, err
	}
...
// constructMessageLink parses a Telegram message URL and returns a canonical t.me link.
func constructMessageLink(url string) (string, error) {
