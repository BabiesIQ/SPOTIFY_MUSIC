...
func (db *Database) AddAuthUser(chatID, userID int64) error {
	...
	if err == nil {
		db.authCache.Delete(convertToKey(chatID))
	}
	return err
}
...
func (db *Database) IsAuthUser(chatID, userID int64) bool {
	admins, err := cache.LoadChatAdminIDs(chatID)
	if err != nil || admins == nil {
		admins = []int64{}
	}

	if sliceContains(admins, userID) {
		return true
	}

	users := db.GetAuthUsers(chatID)
	return sliceContains(users, userID)
}
...
func (db *Database) IsAdmin(chatID, userID int64) bool {
	...
	return sliceContains(admins, userID)
}
