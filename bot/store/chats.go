...
func (db *Database) SetPlayType(chatID int64, playType int) error {
	...
	if err == nil {
		db.chatCache.Delete(convertToKey(chatID))
	}
	return err
}
...
func (db *Database) SetAdminMode(chatID int64, adminModeState string) error {
	ctx, cancel := db.ctx()
	defer cancel()

	_, err := db.chatDB.UpdateOne(ctx, bson.M{"_id": chatID}, bson.M{"$set": bson.M{"admin_mode": adminModeState}}, options.UpdateOne().SetUpsert(true))
	if err == nil {
		db.chatCache.Delete(convertToKey(chatID))
	}
	return err
...
func (db *Database) GetAllChats() ([]int64, error) {
	...
	for cursor.Next(ctx) {
		...
		db.chatCache.Set(convertToKey(doc.ID), &doc)
	}
