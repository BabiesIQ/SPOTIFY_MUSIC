...
func sendRequest(method, fullURL string, body io.Reader, headers map[string]string) (*http.Response, error) {
	...
	errMsg := redactSensitiveInfo(reqErr.Error())
	return nil, fmt.Errorf("request failed: %s", errMsg)
}

// redactSensitiveInfo removes the API key from error messages.
...
// isRetryableError determines if an error is temporary and thus worth retrying.
...
// createUniqueName creates a pseudo-random filename using a combination of the current timestamp and a random number.
func createUniqueName(ext string) string {
	n, _ := rand.Int(rand.Reader, big.NewInt(99999))
	return fmt.Sprintf("%d_%05d%s", time.Now().UnixNano(), n.Int64(), ext)
}

// resolveFilename safely determines a valid filename for a download.
func resolveFilename(urlStr, contentDisp string) string {
	if filename := parseFilename(contentDisp); filename != "" {
	...
	return filepath.Join(config.DownloadsDir, createUniqueName(".tmp"))
}
...
// pullFile downloads a file from a URL and saves it to a local path.
func pullFile(urlStr, fileName string, overwrite bool) (string, error) {
	if urlStr == "" {
	...
	if fileName == "" {
		fileName = resolveFilename(urlStr, resp.Header.Get("Content-Disposition"))
	}
	...
	if err := saveToFile(tempPath, resp.Body); err != nil {
		return "", err
	}
