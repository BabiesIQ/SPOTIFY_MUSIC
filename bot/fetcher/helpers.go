...
// createDownload creates and validates a new download instance.
func createDownload(track utils.TrackInfo) (*download, error) {
	if track.CdnURL == "" {
...
// parseFilename parses the Content-Disposition header to extract the original filename.
func parseFilename(contentDisp string) string {
	if contentDisp == "" {
		return ""
	}
